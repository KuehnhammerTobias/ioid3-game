	...
	if ( CG_LocalPlayerState(attacker) ) {
		...
		for (i = 0; i < CG_MaxSplitView(); i++) {
			...
			if (!(cg_singlePlayer.integer && cg_cameraOrbit.integer)) {
				CG_CenterPrint( i, s, SCREEN_HEIGHT * 0.30, qfalse, 2 );
			} 
#else
			CG_CenterPrint( i, s, SCREEN_HEIGHT * 0.30, qfalse, 2 );
#endif
...
static void CG_UseItem( centity_t *cent ) {
	...
	for (i = 0; i < CG_MaxSplitView(); i++) {
		...
		if ( !itemNum ) {
			CG_CenterPrint( i, "No item to use", SCREEN_HEIGHT * 0.30, qfalse, 1 );
		} else {
			item = BG_FindItemForHoldable( itemNum );
			CG_CenterPrint( i, va("Use %s", item->pickup_name), SCREEN_HEIGHT * 0.30, qfalse, 1 );
		}
	}
